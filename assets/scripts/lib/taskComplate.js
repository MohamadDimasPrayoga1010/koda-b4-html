...
export async function initTaskComplate() {
  const $btnTaskComplate = $("#btnTaskComplate");
  const $taskComplate = $("#taskComplate");
  $taskComplate.hide();

  async function renderTaskComplate() {
  ...
  $btnTaskComplate.on("click", async function () {
    $taskComplate.toggle();
    await renderTaskComplate();
  });
  ...
  $(document).on(
    ...
    async function () {
      ...
      if (task.subtask?.length > 0) {
        task.subtask.forEach((s) => (s.completed = false));
      }
